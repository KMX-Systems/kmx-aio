...
* **[KNXnet/IP](https://en.wikipedia.org/wiki/KNX)**: Feature-gated building-automation stack implemented in-tree, with a `constexpr` cEMI and datapoint codec whose golden wire vectors are checked by `static_assert`, an I/O-free tunnelling session state machine, and an injected transport contract shared by both executor models.
...
| [KNXnet/IP](documentation/features/knx.md) | Building automation: lighting, blinds, HVAC and metering over KNX group communication |
