...
test_bin="$(find "$qbs_build_root/debug" -type f -name kmx-aio-test -not -path '*/install-root/*' -print -quit 2>/dev/null || true)"
if [[ -z "$test_bin" ]]; then
...
if [[ "$run_samples" == "true" ]]; then
    ...
    sample_server="$(find "$qbs_build_root/debug" -type f -name sample-someip-echo-server -not -path '*/install-root/*' -print -quit 2>/dev/null || true)"
    sample_client="$(find "$qbs_build_root/debug" -type f -name sample-someip-echo-client -not -path '*/install-root/*' -print -quit 2>/dev/null || true)"
    sample_publisher="$(find "$qbs_build_root/debug" -type f -name sample-someip-event-publisher -not -path '*/install-root/*' -print -quit 2>/dev/null || true)"
    sample_subscriber="$(find "$qbs_build_root/debug" -type f -name sample-someip-event-subscriber -not -path '*/install-root/*' -print -quit 2>/dev/null || true)"
    sample_diagnostics="$(find "$qbs_build_root/debug" -type f -name sample-someip-diagnostics -not -path '*/install-root/*' -print -quit 2>/dev/null || true)"
