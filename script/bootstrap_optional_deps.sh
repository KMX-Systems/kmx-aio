...
Usage:
    script/bootstrap_optional_deps.sh [options]
...
Examples:
    ...
    bash script/bootstrap_optional_deps.sh --af-xdp --quic --opc-ua --someip
USAGE
}
...
if [[ "${run_accelerators}" == "true" ]]; then
    run_script feature/cuda/install-dependencies.sh
fi

if [[ "${run_af_xdp}" == "true" ]]; then
    run_script feature/af_xdp/install-dependencies.sh
fi

if [[ "${run_avb}" == "true" ]]; then
    run_script feature/avb/install-dependencies.sh
fi

if [[ "${run_spdk}" == "true" ]]; then
    run_script feature/spdk/install-dependencies.sh
fi

if [[ "${run_v4l2}" == "true" ]]; then
    run_script feature/v4l2/install-dependencies.sh
fi

if [[ "${run_quic}" == "true" ]]; then
    run_script feature/quic/install-dependencies.sh
fi

if [[ "${run_opc_ua}" == "true" ]]; then
    run_script feature/opc_ua/install-dependencies.sh
fi

if [[ "${run_someip}" == "true" ]]; then
    run_script feature/someip/install-dependencies.sh
fi

if [[ "${run_cuda_check}" == "true" ]]; then
    run_script feature/cuda/check_env.sh
fi
