/// @file src/kmx/aio/sample/v4l2/capture/manager_model_test.cpp
...
namespace kmx::aio::test::sample::v4l2::capture::manager_model_test
{
    namespace readiness_v4l2 = kmx::aio::sample::v4l2::capture;
    namespace completion_v4l2 = kmx::aio::sample::v4l2::completion_capture;

    using readiness_v4l2::frame_result;
    using readiness_v4l2::step_results;

    TEST_CASE("readiness and completion v4l2 models agree on frame accounting", "[v4l2][readiness][completion][model]")
    {
        static constexpr std::array<frame_result, 4u> readiness_frames {
            frame_result {.recv_ok = true, .frame_ok = true, .bytes_used = 512u},
            frame_result {.recv_ok = true, .frame_ok = false, .bytes_used = 1024u},
            frame_result {.recv_ok = false, .frame_ok = true, .bytes_used = 2048u},
            frame_result {.recv_ok = true, .frame_ok = true, .bytes_used = 1536u},
        };

        static constexpr std::array<completion_v4l2::frame_result, 4u> completion_frames {
            completion_v4l2::frame_result {.recv_ok = true, .frame_ok = true, .bytes_used = 512u},
            completion_v4l2::frame_result {.recv_ok = true, .frame_ok = false, .bytes_used = 1024u},
            completion_v4l2::frame_result {.recv_ok = false, .frame_ok = true, .bytes_used = 2048u},
            completion_v4l2::frame_result {.recv_ok = true, .frame_ok = true, .bytes_used = 1536u},
        };

        step_results readiness_step {};
        readiness_step.device_open_ok = true;
        readiness_step.capture_create_ok = true;
        readiness_step.frames = readiness_frames;

        completion_v4l2::step_results completion_step {};
