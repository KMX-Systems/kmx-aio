/// @file src/kmx/aio/knx/tunnelling_session_test.cpp
/// @brief Unit tests for the KNX tunnelling session state machine: acks, retries, heartbeats, connect and disconnect.
...
namespace kmx::aio::test::knx::tunnelling_session_test
{
    ...
    TEST_CASE("knx session consumes a complete tunnelling ack packet", "[knx][session][integration]")
    {
        ...
        const std::array<std::uint8_t, 10u> packet {
            0x06u, 0x10u, 0x04u, 0x21u, 0x00u, 0x0Au, 0x04u, 0x03u, 0x05u, 0x00u,
        };
    ...
    TEST_CASE("knx session rebuilds an identical retry packet", "[knx][session][integration]")
    {
        ...
        CHECK(std::equal(session.active_request_packet().begin(), session.active_request_packet().end(), original.begin()));
    ...
    TEST_CASE("knx session closes after a matching disconnect response", "[knx][session][integration]")
    {
        ...
        const std::array<std::uint8_t, 8u> packet {0x06u, 0x10u, 0x02u, 0x0Au, 0x00u, 0x08u, 0x03u, 0x00u};
    ...
    TEST_CASE("knx session preserves state for an invalid disconnect response", "[knx][session][unit]")
    {
        ...
        const std::array<std::uint8_t, 8u> packet {0x06u, 0x10u, 0x02u, 0x0Au, 0x00u, 0x08u, 0x04u, 0x00u};
        std::array<std::uint8_t, 16u> disconnect_request {};
        REQUIRE(session.prepare_disconnect_request_packet(disconnect_request).has_value());
    ...
    TEST_CASE("knx session rejects a failed or mismatched heartbeat", "[knx][session][unit]")
    {
        ...
        const auto failed = session.on_connectionstate_response(connectionstate_response_frame {3u, connect_status::knx_connection});
        REQUIRE(!failed.has_value());
        CHECK(failed.error() == make_error_code(error::connection_failed));
        CHECK(session.state() == session_state::connected);

        const auto mismatched = session.on_connectionstate_response(connectionstate_response_frame {4u, connect_status::no_error});
    ...
    TEST_CASE("knx session resets heartbeat failures after success", "[knx][session][unit]")
    {
        ...
        REQUIRE(session.on_connect_response(connect_response_frame {3u, connect_status::no_error, {}}).has_value());
        REQUIRE(!session.on_connectionstate_response(connectionstate_response_frame {3u, connect_status::connection_type}).has_value());
        REQUIRE(session.heartbeat_failures() == 1u);
        REQUIRE(session.on_connectionstate_response(connectionstate_response_frame {3u, connect_status::no_error}).has_value());
    ...
    {
        tunnelling_session session {
            {.max_retries = 2u, .ack_timeout_ms = 1000u, .heartbeat_failure_limit = 3u, .inactivity_timeout_ms = 100u}};
        REQUIRE(session.on_connect_response(connect_response_frame {3u, connect_status::no_error, {}}).has_value());
    ...
    TEST_CASE("knx session records activity after valid datagram ingress", "[knx][session][integration]")
    {
        ...
        REQUIRE(connection::encode_connectionstate_response_packet(heartbeat, connectionstate_response_frame {3u, connect_status::no_error})
                    .has_value());
    ...
    TEST_CASE("knx session dispatches and answers a raw heartbeat request", "[knx][session][integration]")
    {
        ...
        REQUIRE(connection::encode_connectionstate_request_packet(request_packet, connectionstate_request_frame {3u}).has_value());
    ...
    TEST_CASE("knx response preparation does not write rejected requests", "[knx][session][unit]")
    {
        ...
        REQUIRE(!result.has_value());
        CHECK(result.error() == make_error_code(error::sequence_error));
        CHECK(std::all_of(response_packet.begin(), response_packet.end(), [](const std::uint8_t value) { return value == 0xA5u; }));
    }
    ...
    TEST_CASE("knx session dispatches and answers a raw disconnect request", "[knx][session][integration]")
    {
        ...
        std::array<std::uint8_t, 16u> request_packet {};
        REQUIRE(connection::encode_disconnect_request_packet(request_packet, disconnect_request_frame {3u}).has_value());
    ...
    TEST_CASE("knx session dispatches a raw disconnect response while closing", "[knx][session][integration]")
    {
        ...
        std::array<std::uint8_t, 8u> response_packet {};
        REQUIRE(connection::encode_disconnect_response_packet(response_packet, disconnect_response_frame {3u, connect_status::no_error})
                    .has_value());
    ...
    TEST_CASE("knx session consumes a heartbeat response packet", "[knx][session][integration]")
    {
        ...
        REQUIRE(connection::encode_connectionstate_response_packet(packet, connectionstate_response_frame {3u, connect_status::no_error})
                    .has_value());
    ...
    TEST_CASE("knx session reset recovers every terminal connected state", "[knx][session][unit]")
    {
        ...
        REQUIRE(heartbeat_session.on_connect_response(connect_response_frame {3u, connect_status::no_error, {}}).has_value());
        REQUIRE(!heartbeat_session.on_connectionstate_response(connectionstate_response_frame {3u, connect_status::connection_type})
                     .has_value());
        ...
        REQUIRE(inactivity_session.on_connect_response(connect_response_frame {3u, connect_status::no_error, {}}).has_value());
        ...
        REQUIRE(disconnect_session.on_connect_response(connect_response_frame {3u, connect_status::no_error, {}}).has_value());
