...
namespace kmx::aio::test::knx::frame_reassembler_test
{
    namespace kn = kmx::aio::knx;
    using kn::error;
    using kn::make_error_code;
    using reassembler = kn::detail::frame_reassembler;
    ...
    namespace detail
    {
        ...
        void feed(reassembler& value, const cspan_uint8_t octets) noexcept(false)
        {
        ...
        [[nodiscard]] octets_t take(reassembler& value) noexcept(false)
        {
        ...
        [[nodiscard]] bool waiting(reassembler& value) noexcept(false)
        {
            const auto frame = value.next();
    ...
    TEST_CASE("knx frame reassembler makes room behind an unfinished frame and forgets on reset", "[knx][tcp][unit]")
    {
        ...
        CHECK(value.writable().size() == (reassembler::capacity - 100u));
        value.reset();
        CHECK(!value.partial());
        CHECK(value.writable().size() == reassembler::capacity);
    }
}
