/// @file src/kmx/aio/knx/secure/detail/crypto_test.cpp
/// @brief The KNX IP Secure crypto backend and composition against FIPS, RFC, AN159 and xknx known answers.
...
namespace kmx::aio::test::knx::secure::detail::crypto_test
{
    ...
    namespace detail
    {
        ...
        struct routing_example
        {
            ...
            /// @brief The example as the composition protects it; the payload is sealed and opened in place.
            [[nodiscard]] kd::message message() noexcept
            {
                return {.block_0 = block_0(), .counter_0 = counter_0(), .associated_data = associated, .payload = payload};
            }
        };
        ...
        [[nodiscard]] bool refused_after_alteration(const routing_example& example, kd::block_t mac, const int alter) noexcept(false)
        {
            ...
            const auto opened =
                kd::open({kd::evp_backend(), example.key},
                         {.block_0 = block_0, .counter_0 = counter_0, .associated_data = associated, .payload = payload}, mac);
    ...
    TEST_CASE("knx secure composition reproduces the AN159 routing example", "[knx][secure][crypto][unit]")
    {
        ...
        const auto mac = kd::seal({backend, example.key}, example.message());
        ...
        REQUIRE(kd::open({backend, example.key}, example.message(), *mac).has_value());
    ...
    TEST_CASE("knx secure composition reproduces the xknx session wrappers", "[knx][secure][crypto][unit]")
    {
        ...
        const auto wrapped_header = detail::hex("06 10 09 50 00 3e 00 01");
        const auto wrapped_mac = kd::seal({backend, *session_key}, {.block_0 = kd::wrapper_block_0(first, client_serial, tag, 24u),
                                                                    .counter_0 = kd::wrapper_counter_0(first, client_serial, tag),
                                                                    .associated_data = wrapped_header,
        ...
        const auto status_header = detail::hex("06 10 09 50 00 2e 00 01");
        REQUIRE(kd::open({backend, *session_key},
                         {.block_0 = kd::wrapper_block_0(first, server_serial, tag, 8u),
                          .counter_0 = kd::wrapper_counter_0(first, server_serial, tag),
                          .associated_data = status_header,
                          .payload = status},
    ...
    {
        detail::routing_example example {};
        const auto mac = kd::seal({kd::evp_backend(), example.key}, example.message());
        REQUIRE(mac.has_value());
    ...
    TEST_CASE("knx secure composition reports backend failures as crypto_failure", "[knx][secure][crypto][unit]")
    {
        ...
        CHECK(
            kd::seal({detail::failing_backend, example.key}, {.associated_data = example.associated, .payload = example.payload}).error() ==
            failure);
        CHECK(kd::open({detail::failing_backend, example.key}, {.associated_data = example.associated, .payload = example.payload}, {})
                  .error() == failure);
        ...
        CHECK(kd::open({decrypting_only, example.key}, {.associated_data = example.associated, .payload = example.payload}, {}).error() ==
              failure);
