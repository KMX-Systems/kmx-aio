/// @file src/kmx/aio/knx/tunnelling_client_stream_test.cpp
...
namespace kmx::aio::test::knx::tunnelling_client_stream_test
{
    ...
    namespace detail
    {
        ...
        struct tcp_fixture
        {
            stand_in_transport transport;
            sockaddr_in server = server_address();
            tunnelling_client client {
                transport,
                reinterpret_cast<const sockaddr*>(&server),
                sizeof(server),
                {.config = {.connectionstate_timeout_ms = 1'000u, .heartbeat_failure_limit = 2u}, .clock_now = clock_now}};
            completion::executor executor;

            tcp_fixture() noexcept
            {
                now_ms = 0u;
    ...
    TEST_CASE_METHOD(detail::tcp_fixture, "knx tunnelling client over a stream opens the connection and asks with TCP HPAIs",
                     "[knx][client][tcp][unit]")
    {
    ...
    TEST_CASE_METHOD(detail::tcp_fixture, "knx tunnelling client over a stream sends without waiting for an acknowledgement",
                     "[knx][client][tcp][unit]")
    {
        bool first {};
        bool second {};
    ...
    TEST_CASE_METHOD(detail::tcp_fixture, "knx tunnelling client over a stream neither acknowledges indications nor orders them",
                     "[knx][client][tcp][unit]")
    {
    ...
    TEST_CASE_METHOD(detail::tcp_fixture, "knx tunnelling client over a stream reports an unanswered heartbeat from poll",
                     "[knx][client][tcp][unit]")
    {
    ...
    TEST_CASE_METHOD(detail::tcp_fixture, "knx tunnelling client over a stream sends and beats beside a waiting receive",
                     "[knx][client][tcp][unit]")
    {
        bool overlapped {};
        bool sent {};
    ...
    TEST_CASE_METHOD(detail::tcp_fixture, "knx tunnelling client over a stream reconnects over a new connection",
                     "[knx][client][tcp][unit]")
    {
        bool disconnected {};
    ...
    TEST_CASE_METHOD(detail::tcp_fixture, "knx tunnelling client over a stream ends the tunnel with its connection",
                     "[knx][client][tcp][unit]")
    {
    ...
    TEST_CASE_METHOD(detail::tcp_fixture, "knx tunnelling client over a stream makes one connect attempt", "[knx][client][tcp][unit]")
    {
    ...
    TEST_CASE_METHOD(detail::tcp_fixture, "knx tunnelling client over a stream refuses an IPv6 connect", "[knx][client][tcp][unit]")
    {
        std::error_code failure {};
        auto run = [&]() -> task<void>
        {
    ...
    TEST_CASE_METHOD(detail::tcp_fixture, "knx tunnelling client over a stream disconnects once when a heartbeat answer comes first",
                     "[knx][client][tcp][unit]")
    {
        bool beat {};
        bool disconnected {};
