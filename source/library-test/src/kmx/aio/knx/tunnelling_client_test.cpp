/// @file src/kmx/aio/knx/tunnelling_client_test.cpp
/// @brief The KNXnet/IP tunnelling client over a scripted datagram transport: lifecycle, retries, heartbeats, peer checks.
...
namespace kmx::aio::test::knx::tunnelling_client_test
{
    ...
    namespace detail
    {
        ...
        public:
            ...
            void enqueue(std::vector<std::uint8_t> packet) { responses_.push_back(std::move(packet)); }
        ...
        /// @brief The transport, client, loop and connect request one scripted exchange runs against.
        struct scenario
        {
            /// @brief The scripted transport the client drives.
            loopback_transport& transport;
            /// @brief The client under test.
            tunnelling_client& client;
            /// @brief The loop the exchange runs on; the exchange stops it when it is done.
            completion::executor& executor;
            /// @brief The connection the exchange asks for.
            const connect_request_frame& request = loopback_connect_request;
        };
        ...
        struct loopback_fixture
        {
            ...
            /// @brief Returns this fixture's transport, client and loop, asking for @ref loopback_connect_request.
            [[nodiscard]] scenario scene() noexcept { return {transport, client, executor}; }
        ...
        task<void> run_loopback_lifecycle(const scenario scene, const cspan_uint8_t cemi, bool& succeeded) noexcept(false)
        {
            const auto connected = co_await scene.client.connect(scene.request);
            if (!connected)
            {
                scene.executor.stop();
                co_return;
            ...
            const auto sent = co_await scene.client.send(cemi);
            if (!sent)
            {
                scene.executor.stop();
                co_return;
            ...
            const auto disconnected = co_await scene.client.disconnect();
        ...
        task<void> retry_timed_out_request(const scenario scene, const cspan_uint8_t cemi, bool& succeeded) noexcept(false)
        {
            const auto connected = co_await scene.client.connect(scene.request);
            if (!connected)
            {
            ...
            scene.transport.timeout_next_receive = true;
            const auto sent = co_await scene.client.send(cemi);
        ...
        task<void> ignore_stale_ack(const scenario scene, const cspan_uint8_t stale_ack, bool& succeeded) noexcept(false)
        {
            if (!(co_await scene.client.connect(scene.request)).has_value())
            {
                scene.executor.stop();
                co_return;
            }

            scene.transport.enqueue(std::vector<std::uint8_t>(stale_ack.begin(), stale_ack.end()));
            succeeded = (co_await scene.client.send(sample_cemi)).has_value();
            scene.executor.stop();
        }

        /// @brief Connects, then disconnects with the first receive timed out, so the request has to be retried.
        task<void> retry_timed_out_disconnect(const scenario scene, bool& succeeded) noexcept(false)
        {
            if (!(co_await scene.client.connect(scene.request)).has_value())
            {
                scene.executor.stop();
            ...
            scene.transport.timeout_next_receive = true;
            succeeded = (co_await scene.client.disconnect()).has_value();
            scene.executor.stop();
        }

        /// @brief Connects and completes one heartbeat exchange.
        task<void> complete_heartbeat(const scenario scene, bool& succeeded) noexcept(false)
        {
            if (!(co_await scene.client.connect(scene.request)).has_value())
            {
                scene.executor.stop();
            ...
            succeeded = (co_await scene.client.heartbeat()).has_value();
            scene.executor.stop();
        }

        /// @brief Connects, then heartbeats against a transport that refuses, and records what came back.
        task<void> propagate_heartbeat_failure(const scenario scene, bool& failed, bool& stayed_connected) noexcept(false)
        {
            if (!(co_await scene.client.connect(scene.request)).has_value())
            {
                scene.executor.stop();
            ...
            scene.transport.heartbeat_failure = true;
            const auto result = co_await scene.client.heartbeat();
            failed = !result.has_value() && result.error() == make_error_code(error::connection_failed);
            stayed_connected = scene.client.state() == session_state::connected;
            scene.executor.stop();
        }

        /// @brief Connects against a transport that refuses, and records the error and the resulting state.
        task<void> propagate_connect_failure(const scenario scene, bool& failed) noexcept(false)
        {
            const auto result = co_await scene.client.connect(scene.request);
            failed = !result.has_value();
            CHECK(result.error() == make_error_code(error::connection_failed));
            CHECK(scene.client.state() == session_state::idle);
            scene.executor.stop();
        }

        /// @brief Connects, then heartbeats with the first receive timed out, so the request has to be retried.
        task<void> retry_timed_out_heartbeat(const scenario scene, bool& succeeded) noexcept(false)
        {
            if (!(co_await scene.client.connect(scene.request)).has_value())
            {
                scene.executor.stop();
        ...
        task<void> escalate_heartbeat_failures(const scenario scene, bool& terminal) noexcept(false)
        {
            if (!(co_await scene.client.connect(scene.request)).has_value())
            {
                scene.executor.stop();
            ...
            scene.transport.heartbeat_failure = true;
            static_cast<void>(co_await scene.client.heartbeat());
            static_cast<void>(co_await scene.client.heartbeat());
            const auto result = co_await scene.client.heartbeat();
            terminal = !result.has_value() && result.error() == make_error_code(error::heartbeat_failed) &&
                       scene.client.state() == session_state::closed;
            scene.executor.stop();
        }

        /// @brief Calls every public operation on a shut-down client and records that each one was refused.
        task<void> reject_after_shutdown(const scenario scene, const cspan_uint8_t cemi, bool& rejected) noexcept(false)
        {
            const auto send_result = co_await scene.client.send(cemi);
            const auto heartbeat_result = co_await scene.client.heartbeat();
            const auto disconnect_result = co_await scene.client.disconnect();
            const auto connect_result = co_await scene.client.connect(scene.request);
            rejected =
        ...
        task<void> reject_receive_after_shutdown(const scenario scene, bool& rejected) noexcept(false)
        {
            const auto datagram = co_await scene.client.receive_datagram();
            const auto cemi = co_await scene.client.receive_cemi();
        ...
        task<void> reset_and_reconnect(const scenario scene, bool& reconnected) noexcept(false)
        {
            if (!(co_await scene.client.connect(scene.request)).has_value())
            {
                scene.executor.stop();
            ...
            scene.client.shutdown();
            scene.client.reset();
            scene.transport.advertised_data_port = 3673u;
            reconnected = (co_await scene.client.connect(scene.request)).has_value() && scene.client.state() == session_state::connected &&
                          (co_await scene.client.send(sample_cemi)).has_value();
            scene.executor.stop();
        }

        /// @brief Connects, then sends against a transport whose write fails, and records the error that came back.
        task<void> preserve_send_error(const scenario scene, const cspan_uint8_t cemi, const std::error_code& transport_error,
                                       bool& send_preserved) noexcept(false)
        {
            if (!(co_await scene.client.connect(scene.request)).has_value())
            {
                scene.executor.stop();
            ...
            scene.transport.send_error = transport_error;
            const auto result = co_await scene.client.send(cemi);
        ...
        task<void> preserve_heartbeat_error(const scenario scene, const std::error_code& transport_error,
                                            bool& heartbeat_preserved) noexcept(false)
        {
            if (!(co_await scene.client.connect(scene.request)).has_value())
            {
                scene.executor.stop();
            ...
            scene.transport.receive_error = transport_error;
            const auto result = co_await scene.client.heartbeat();
        ...
        task<void> preserve_disconnect_send_error(const scenario scene, const std::error_code& transport_error,
                                                  bool& send_preserved) noexcept(false)
        {
            if (!(co_await scene.client.connect(scene.request)).has_value())
            {
            ...
            scene.transport.send_error = transport_error;
            const auto result = co_await scene.client.disconnect();
            send_preserved = !result.has_value() && result.error() == transport_error;
            scene.executor.stop();
        }

        /// @brief Connects, then disconnects against a transport whose read fails, and records the error that came back.
        task<void> preserve_disconnect_receive_error(const scenario scene, const std::error_code& transport_error,
                                                     bool& receive_preserved) noexcept(false)
        {
            if (!(co_await scene.client.connect(scene.request)).has_value())
            {
                scene.executor.stop();
        ...
        task<void> reject_oversized_disconnect_receive(const scenario scene, bool& rejected) noexcept(false)
        {
            if (!(co_await scene.client.connect(scene.request)).has_value())
            {
                scene.executor.stop();
            ...
            scene.transport.oversized_receive = true;
            const auto result = co_await scene.client.disconnect();
        ...
        task<void> observe_disconnect_cancellation(const scenario scene, std::stop_source& source, bool& cancelled) noexcept(false)
        {
            if (!(co_await scene.client.connect(scene.request)).has_value())
            {
                scene.executor.stop();
        ...
        task<void> reject_empty_cemi(const scenario scene, const cspan_uint8_t cemi, bool& rejected) noexcept(false)
        {
            const connect_request_frame request {
                .control_endpoint = hpai {ipv4_endpoint {{127u, 0u, 0u, 1u}, 3671u}, 0x01u},
                .data_endpoint = hpai {ipv4_endpoint {{127u, 0u, 0u, 1u}, 3672u}, 0x01u},
            };
            if (!(co_await scene.client.connect(request)).has_value())
            {
                scene.executor.stop();
                co_return;
            }
        ...
        task<void> connect_and_send_over_ipv6(const scenario scene, const ipv6_connect_request_frame& request,
                                              bool& succeeded) noexcept(false)
        {
            const auto connected = co_await scene.client.connect(request);
            const auto sent = connected ? co_await scene.client.send(sample_cemi) : expected_void_t {std::unexpected(connected.error())};
        ...
        task<void> reject_control_peer_tunnelling(const scenario scene, const cspan_uint8_t packet, bool& rejected) noexcept(false)
        {
            if (!(co_await scene.client.connect(scene.request)).has_value())
            {
                scene.executor.stop();
            ...
            scene.transport.enqueue(std::vector<std::uint8_t>(packet.begin(), packet.end()));
            const auto result = co_await scene.client.receive_datagram();
        ...
        task<void> receive_owning_datagram(const scenario scene, bool& received_ack) noexcept(false)
        {
            const auto result = co_await scene.client.receive_datagram();
            if (result.has_value())
            {
        ...
        task<void> receive_owning_cemi(const scenario scene, const cspan_uint8_t cemi, const cspan_uint8_t packet,
                                       bool& matched) noexcept(false)
        {
            const connect_request_frame connect_request {
                .control_endpoint = hpai {ipv4_endpoint {{127u, 0u, 0u, 1u}, 3671u}, 0x01u},
                .data_endpoint = hpai {ipv4_endpoint {{127u, 0u, 0u, 1u}, 3672u}, 0x01u},
            };
            if (!(co_await scene.client.connect(connect_request)).has_value())
            {
                scene.executor.stop();
                co_return;
            }
            ...
            scene.transport.enqueue(std::vector<std::uint8_t>(packet.begin(), packet.end()));
            const auto result = co_await scene.client.receive_cemi();
        ...
        task<void> receive_owning_cemi_copy(const scenario scene, const cspan_uint8_t request,
                                            std::vector<std::uint8_t>& received_payload) noexcept(false)
        {
            const connect_request_frame connect_request {
                .control_endpoint = hpai {ipv4_endpoint {{127u, 0u, 0u, 1u}, 3671u}, 0x01u},
                .data_endpoint = hpai {ipv4_endpoint {{127u, 0u, 0u, 1u}, 3672u}, 0x01u},
            ...
            scene.transport.enqueue(std::vector<std::uint8_t>(request.begin(), request.end()));
            received_payload = *(co_await scene.client.receive_cemi());
            scene.executor.stop();
        }

        /// @brief Connects and feeds the transport the same indication twice, so the second is suppressed.
        task<void> suppress_duplicate_indication(const scenario scene, const cspan_uint8_t indication, bool& first_received,
                                                 bool& duplicate_suppressed) noexcept(false)
        {
            if (!(co_await scene.client.connect(connect_request_frame {
                      .control_endpoint = hpai {ipv4_endpoint {{127u, 0u, 0u, 1u}, 3671u}, 0x01u},
                      .data_endpoint = hpai {ipv4_endpoint {{127u, 0u, 0u, 1u}, 3672u}, 0x01u},
                  }))
                     .has_value())
            ...
            scene.transport.enqueue(std::vector<std::uint8_t>(indication.begin(), indication.end()));
            scene.transport.enqueue(std::vector<std::uint8_t>(indication.begin(), indication.end()));
            first_received = (co_await scene.client.receive_cemi()).has_value();
            const auto duplicate = co_await scene.client.receive_cemi();
        ...
        task<void> reject_out_of_order_indication(const scenario scene, const cspan_uint8_t first, const cspan_uint8_t out_of_order,
                                                  bool& rejected) noexcept(false)
        {
            if (!(co_await scene.client.connect(connect_request_frame {
                      .control_endpoint = hpai {ipv4_endpoint {{127u, 0u, 0u, 1u}, 3671u}, 0x01u},
                      .data_endpoint = hpai {ipv4_endpoint {{127u, 0u, 0u, 1u}, 3672u}, 0x01u},
            ...
            scene.transport.enqueue(std::vector<std::uint8_t>(first.begin(), first.end()));
            scene.transport.enqueue(std::vector<std::uint8_t>(out_of_order.begin(), out_of_order.end()));
            if (!(co_await scene.client.receive_cemi()).has_value())
            {
                scene.executor.stop();
                co_return;
            }
        ...
        task<void> skip_control_before_cemi(const scenario scene, const cspan_uint8_t request, const cspan_uint8_t ack,
                                            bool& received) noexcept(false)
        {
            if (!(co_await scene.client.connect(connect_request_frame {
                      .control_endpoint = hpai {ipv4_endpoint {{127u, 0u, 0u, 1u}, 3671u}, 0x01u},
                      .data_endpoint = hpai {ipv4_endpoint {{127u, 0u, 0u, 1u}, 3672u}, 0x01u},
            ...
            scene.transport.enqueue(std::vector<std::uint8_t>(ack.begin(), ack.end()));
            scene.transport.enqueue(std::vector<std::uint8_t>(request.begin(), request.end()));
            const auto result = co_await scene.client.receive_cemi();
        ...
        task<void> process_heartbeat_before_cemi(const scenario scene, const cspan_uint8_t request, bool& received) noexcept(false)
        {
            const connect_request_frame connect_request {
                .control_endpoint = hpai {ipv4_endpoint {{127u, 0u, 0u, 1u}, 3671u}, 0x01u},
                .data_endpoint = hpai {ipv4_endpoint {{127u, 0u, 0u, 1u}, 3672u}, 0x01u},
            ...
            REQUIRE(
                connection::encode_connectionstate_response_packet(heartbeat, connectionstate_response_frame {3u, connect_status::no_error})
                    .has_value());
            scene.transport.enqueue(std::vector<std::uint8_t>(heartbeat.begin(), heartbeat.end()));
            scene.transport.enqueue(std::vector<std::uint8_t>(request.begin(), request.end()));
            received = (co_await scene.client.receive_cemi()).has_value();
            scene.executor.stop();
        }

        /// @brief Connects and feeds the transport a tunnelling request carrying another channel id.
        task<void> reject_cross_channel_cemi(const scenario scene, const cspan_uint8_t request, bool& rejected) noexcept(false)
        {
            const connect_request_frame connect_request {
                .control_endpoint = hpai {ipv4_endpoint {{127u, 0u, 0u, 1u}, 3671u}, 0x01u},
                .data_endpoint = hpai {ipv4_endpoint {{127u, 0u, 0u, 1u}, 3672u}, 0x01u},
            ...
            scene.transport.short_send = true;
            scene.transport.enqueue(std::vector<std::uint8_t>(request.begin(), request.end()));
            const auto result = co_await scene.client.receive_cemi();
            rejected = !result.has_value() && result.error() == make_error_code(error::sequence_error);
            scene.executor.stop();
        ...
        task<void> reject_disconnect_control_as_cemi(const scenario scene, const cspan_uint8_t disconnect, bool& rejected) noexcept(false)
        {
            if (!(co_await scene.client.connect(connect_request_frame {
                      .control_endpoint = hpai {ipv4_endpoint {{127u, 0u, 0u, 1u}, 3671u}, 0x01u},
                      .data_endpoint = hpai {ipv4_endpoint {{127u, 0u, 0u, 1u}, 3672u}, 0x01u},
            ...
            scene.transport.enqueue(std::vector<std::uint8_t>(disconnect.begin(), disconnect.end()));
            const auto result = co_await scene.client.receive_cemi();
        ...
        task<void> write_typed_group_value(const scenario scene, bool& succeeded) noexcept(false)
        {
            if (!(co_await scene.client.connect(detail::loopback_connect_request)))
            {
                scene.executor.stop();
                co_return;
            }
            ...
            const auto written = co_await scene.client.write_group_value(group_address {0x0A03u}, value.value());
            succeeded = written.has_value();
            scene.executor.stop();
        }

        /// @brief Connects and issues one group-value read.
        task<void> read_group_value(const scenario scene, bool& succeeded) noexcept(false)
        {
            if (!(co_await scene.client.connect(detail::loopback_connect_request)))
            {
                scene.executor.stop();
            ...
            succeeded = (co_await scene.client.read_group_value(group_address {0x0A03u})).has_value();
            scene.executor.stop();
        }

        /// @brief Connects, feeds the transport an indication, and keeps the decoded telegram.
        task<void> decode_received_telegram(const scenario scene, const std::vector<std::uint8_t>& indication,
                                            std::expected<telegram, std::error_code>& received) noexcept(false)
        {
            if (!(co_await scene.client.connect(detail::loopback_connect_request)))
            {
                scene.executor.stop();
            ...
            scene.transport.enqueue(indication);
            received = co_await scene.client.receive_telegram();
            scene.executor.stop();
        }
    }

    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client completes a loopback lifecycle", "[knx][client][integration]")
    {
        const auto& cemi = sample_cemi;
        bool succeeded {};

        spawn_and_run(detail::run_loopback_lifecycle(scene(), cemi, succeeded));
        CHECK(succeeded);
        CHECK(client.state() == session_state::closed);
        REQUIRE(transport.sent_peers().size() >= 3u);
        const auto& tunnelling_destination = reinterpret_cast<const sockaddr_in&>(transport.sent_peers()[1u]);
        CHECK(tunnelling_destination.sin_family == AF_INET);
        CHECK(ntohs(tunnelling_destination.sin_port) == 3672u);
    }

    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client retries a timed-out request", "[knx][client][integration]")
    {
        const auto& cemi = sample_cemi;
        bool succeeded {};

        spawn_and_run(detail::retry_timed_out_request(scene(), cemi, succeeded));
        CHECK(succeeded);
        CHECK(client.state() == session_state::connected);
    }

    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client ignores a stale tunnelling acknowledgement",
                     "[knx][client][integration]")
    {
        ...
        bool succeeded {};
        spawn_and_run(detail::ignore_stale_ack(scene(), stale_ack, succeeded));
        CHECK(succeeded);
    }

    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client retries a timed-out disconnect", "[knx][client][integration]")
    {
        bool succeeded {};

        spawn_and_run(detail::retry_timed_out_disconnect(scene(), succeeded));
        CHECK(succeeded);
        CHECK(client.state() == session_state::closed);
    }

    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client completes a heartbeat", "[knx][client][integration]")
    {
        bool succeeded {};

        spawn_and_run(detail::complete_heartbeat(scene(), succeeded));
        CHECK(succeeded);
        CHECK(client.state() == session_state::connected);
    }

    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client propagates heartbeat failure", "[knx][client][unit]")
    {
        bool failed {};
        bool stayed_connected {};

        spawn_and_run(detail::propagate_heartbeat_failure(scene(), failed, stayed_connected));
        CHECK(failed);
        CHECK(stayed_connected);
    }

    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client propagates connect failure", "[knx][client][unit]")
    {
        transport.connect_failure = true;
        bool failed {};
        spawn_and_run(detail::propagate_connect_failure(scene(), failed));
        CHECK(failed);
    }

    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client retries a timed-out heartbeat", "[knx][client][integration]")
    {
        bool succeeded {};

        spawn_and_run(detail::retry_timed_out_heartbeat(scene(), succeeded));
        CHECK(succeeded);
        CHECK(client.state() == session_state::connected);
    }

    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client escalates repeated heartbeat failures", "[knx][client][integration]")
    {
        bool terminal {};

        spawn_and_run(detail::escalate_heartbeat_failures(scene(), terminal));
        CHECK(terminal);
    }

    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client rejects operations after shutdown", "[knx][client][unit]")
    {
        const auto& cemi = sample_cemi;
        ...
        spawn_and_run(detail::reject_after_shutdown(scene(), cemi, rejected));
        CHECK(rejected);
    }

    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client honors task cancellation", "[knx][client][unit]")
    {
    ...
    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client rejects a concurrent public operation", "[knx][client][unit]")
    {
    ...
    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client rejects receive APIs after shutdown", "[knx][client][unit]")
    {
        client.shutdown();

        bool rejected {};
        spawn_and_run(detail::reject_receive_after_shutdown(scene(), rejected));
        CHECK(rejected);
    }

    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client can reset and reconnect", "[knx][client][integration]")
    {
        bool reconnected {};

        spawn_and_run(detail::reset_and_reconnect(scene(), reconnected));
        CHECK(reconnected);
        REQUIRE(transport.sent_peers().size() >= 3u);
        const auto& tunnelling_destination = reinterpret_cast<const sockaddr_in&>(transport.sent_peers().back());
    ...
    TEST_CASE("knx tunnelling client preserves transport error codes", "[knx][client][unit]")
    {
        ...
        detail::loopback_transport send_transport;
        ...
        detail::loopback_transport receive_transport;
    ...
    TEST_CASE("knx connected operations preserve transport error codes", "[knx][client][unit]")
    {
        ...
        completion::executor send_executor;
        send_executor.spawn(
            detail::preserve_send_error({send_transport, send_client, send_executor, request}, cemi, transport_error, send_preserved));
        send_executor.run();

        detail::loopback_transport heartbeat_transport;
        ...
        heartbeat_executor.spawn(detail::preserve_heartbeat_error({heartbeat_transport, heartbeat_client, heartbeat_executor, request},
                                                                  transport_error, heartbeat_preserved));
    ...
    TEST_CASE("knx disconnect preserves transport error codes", "[knx][client][unit]")
    {
        ...
        completion::executor send_executor;
        send_executor.spawn(
            detail::preserve_disconnect_send_error({send_transport, send_client, send_executor, request}, transport_error, send_preserved));
        send_executor.run();
        ...
        bool receive_preserved {};
        completion::executor receive_executor;
        receive_executor.spawn(detail::preserve_disconnect_receive_error({receive_transport, receive_client, receive_executor, request},
                                                                         transport_error, receive_preserved));
        receive_executor.run();
    ...
    TEST_CASE_METHOD(detail::loopback_fixture, "knx disconnect rejects an oversized receive count", "[knx][client][unit]")
    {
        bool rejected {};
        spawn_and_run(detail::reject_oversized_disconnect_receive(scene(), rejected));
        CHECK(rejected);
    }

    TEST_CASE_METHOD(detail::loopback_fixture, "knx disconnect observes cancellation", "[knx][client][unit]")
    {
        std::stop_source source;
        bool cancelled {};
        spawn_and_run(std::move(detail::observe_disconnect_cancellation(scene(), source, cancelled)).with_stop_token(source.get_token()));
        CHECK(cancelled);
    }

    TEST_CASE("knx client uses the injected monotonic clock", "[knx][client][integration]")
    {
        detail::now_ms = 42'000u;
        detail::loopback_transport transport;
        sockaddr_storage peer {};
        ...
        tunnelling_client client {transport,
                                  peer,
                                  sizeof(peer),
                                  {.config = {.ack_timeout_ms = 1'000u, .connect_timeout_ms = 7'000u}, .clock_now = &detail::clock_now}};
        const connect_request_frame& request {detail::loopback_connect_request};
        bool succeeded {};
        completion::executor executor;
        auto run = [&]() -> task<void>
        ...
        executor.run();
        CHECK(succeeded);
        CHECK(client.last_activity_ms() == detail::now_ms);
        CHECK(transport.last_receive_deadline == detail::now_ms + 7'000u);
        detail::now_ms = 0u;
    }
    ...
    TEST_CASE("knx client poll closes an inactive session", "[knx][client][integration]")
    {
        ...
        tunnelling_client client {
            ...
            peer,
            sizeof(peer),
            {.config = {.inactivity_timeout_ms = 100u}, .clock_now = &detail::clock_now},
        };
        const connect_request_frame& request {detail::loopback_connect_request};
        bool connected {};
        completion::executor executor;
    ...
    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client rejects an empty cEMI payload", "[knx][client][unit]")
    {
        const std::array<std::uint8_t, 0u> cemi {};
        bool rejected {};
        spawn_and_run(detail::reject_empty_cemi(scene(), cemi, rejected));
        CHECK(rejected);
    }

    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client rejects unsupported HPAI connect metadata", "[knx][client][unit]")
    {
        const connect_request_frame request {
    ...
    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client rejects zero-port connect metadata", "[knx][client][unit]")
    {
    ...
    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client rejects an unexpected peer", "[knx][client][unit]")
    {
    ...
    TEST_CASE("knx tunnelling client connects and sends over IPv6", "[knx][client][integration]")
    {
        ...
        bool succeeded {};
        completion::executor executor;
        executor.spawn(detail::connect_and_send_over_ipv6({transport, client, executor}, request, succeeded));
        executor.run();
        CHECK(succeeded);
    ...
    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client rejects invalid peer metadata", "[knx][client][unit]")
    {
    ...
    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client rejects tunnelling traffic from the control peer",
                     "[knx][client][unit]")
    {
        ...
        REQUIRE(frame::encode_tunnelling_ack_packet(packet, 3u, 7u).has_value());
        bool rejected {};
        spawn_and_run(detail::reject_control_peer_tunnelling(scene(), packet, rejected));
        CHECK(rejected);
    }

    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client rejects a bounded peer-length mismatch", "[knx][client][unit]")
    {
    ...
    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client rejects malformed application datagrams", "[knx][client][unit]")
    {
    ...
    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client rejects an empty datagram", "[knx][client][unit]")
    {
    ...
    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client rejects an oversized receive result", "[knx][client][unit]")
    {
    ...
    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client receives an owning typed datagram", "[knx][client][integration]")
    {
        bool received_ack {};
        ...
        spawn_and_run(detail::receive_owning_datagram(scene(), received_ack));
        CHECK(received_ack);
    }

    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client receives an owning cEMI payload", "[knx][client][integration]")
    {
        const auto& cemi = sample_cemi;
        ...
        spawn_and_run(detail::receive_owning_cemi(scene(), cemi, packet, matched));
        CHECK(matched);
        CHECK(transport.ack_received);
    }

    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client returns an owning cEMI copy", "[knx][client][unit]")
    {
        const auto& cemi = sample_cemi;
        ...
        spawn_and_run(detail::receive_owning_cemi_copy(scene(), request, received_payload));
        CHECK(received_payload == std::vector<std::uint8_t>(cemi.begin(), cemi.end()));
    }

    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client acknowledges but suppresses duplicate indications",
                     "[knx][client][integration]")
    {
        ...
        spawn_and_run(detail::suppress_duplicate_indication(scene(), indication, first_received, duplicate_suppressed));
        CHECK(first_received);
        CHECK(duplicate_suppressed);
        CHECK(transport.ack_received);
    }

    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client rejects out-of-order indications", "[knx][client][integration]")
    {
        ...
        bool rejected {};
        spawn_and_run(detail::reject_out_of_order_indication(scene(), first, out_of_order, rejected));
        CHECK(rejected);
    }

    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client skips control datagrams before cEMI", "[knx][client][integration]")
    {
        const auto& cemi = sample_cemi;
        std::array<std::uint8_t, sample_tunnelling_packet_size> request {};
        REQUIRE(frame::encode_tunnelling_request_packet(request, 3u, 7u, cemi).has_value());
        ...
        bool received {};
        spawn_and_run(detail::skip_control_before_cemi(scene(), request, ack, received));
        CHECK(received);
    }

    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client processes heartbeat before cEMI", "[knx][client][integration]")
    {
        std::array<std::uint8_t, sample_tunnelling_packet_size> request {};
        const auto& cemi = sample_cemi;
        REQUIRE(frame::encode_tunnelling_request_packet(request, 3u, 7u, cemi).has_value());
        ...
        bool received {};
        spawn_and_run(detail::process_heartbeat_before_cemi(scene(), request, received));
        CHECK(received);
    }

    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client rejects cross-channel cEMI", "[knx][client][unit]")
    {
        const auto& cemi = sample_cemi;
        std::array<std::uint8_t, sample_tunnelling_packet_size> request {};
        ...
        bool rejected {};
        spawn_and_run(detail::reject_cross_channel_cemi(scene(), request, rejected));
        CHECK(rejected);
    }

    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client does not expose disconnect control as cEMI", "[knx][client][unit]")
    {
        ...
        bool rejected {};
        spawn_and_run(detail::reject_disconnect_control_as_cemi(scene(), disconnect, rejected));
        CHECK(rejected);
    }
}
...
namespace kmx::aio::test::knx::tunnelling_client_test
{
    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client writes a typed group value", "[knx][client][dpt][integration]")
    {
        bool succeeded {};

        spawn_and_run(detail::write_typed_group_value(scene(), succeeded));
    ...
    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client reads a group value", "[knx][client][dpt][integration]")
    {
        bool succeeded {};

        spawn_and_run(detail::read_group_value(scene(), succeeded));
    ...
    TEST_CASE_METHOD(detail::loopback_fixture, "knx tunnelling client decodes a received telegram", "[knx][client][dpt][integration]")
    {
        ...
        spawn_and_run(detail::decode_received_telegram(scene(), indication, received));
