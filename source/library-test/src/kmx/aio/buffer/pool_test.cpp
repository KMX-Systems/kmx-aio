...
{
    TEST_CASE("buffer::pool initializes with full capacity", "[buffer_pool][initialization]")
    {
        kmx::aio::buffer::pool<int, 10> pool;
    ...
    TEST_CASE("buffer::pool acquires and releases single buffer", "[buffer_pool][acquire][release]")
    {
        kmx::aio::buffer::pool<int, 10> pool;
    ...
    TEST_CASE("buffer::pool tracks allocation state correctly", "[buffer_pool][statistics]")
    {
        kmx::aio::buffer::pool<std::string, 5> pool;

        std::vector<kmx::aio::buffer::handle<std::string>> handles;
    ...
    TEST_CASE("buffer::pool throws when exhausted", "[buffer_pool][exhaustion]")
    {
    ...
    TEST_CASE("buffer::pool supports complex types", "[buffer_pool][types]")
    {
        ...
        kmx::aio::buffer::pool<complex_buffer, 5> pool;
    ...
    TEST_CASE("buffer::handle move semantics", "[buffer_pool][buffer_handle][move]")
    {
        kmx::aio::buffer::pool<int, 10> pool;
    ...
    TEST_CASE("buffer::handle arrow operator", "[buffer_pool][buffer_handle]")
    {
        struct point
        {
            int x = 0;
    ...
    TEST_CASE("buffer::handle explicit reset", "[buffer_pool][buffer_handle][reset]")
    {
        kmx::aio::buffer::pool<int, 10> pool;
    ...
    TEST_CASE("buffer::handle default constructed is invalid", "[buffer_pool][buffer_handle]")
    {
        kmx::aio::buffer::handle<int> handle;
    ...
    TEST_CASE("buffer::pool zero-copy reuse", "[buffer_pool][zero-copy]")
    {
        // Simulate a zero-copy scenario where buffers are reused for I/O
        kmx::aio::buffer::pool<std::vector<std::byte>, 3> pool;

        // Round 1: acquire all buffers
        {
            std::vector<kmx::aio::buffer::handle<std::vector<std::byte>>> handles;
    ...
    TEST_CASE("buffer::pool interleaved acquire-release", "[buffer_pool][fragmentation]")
    {
        kmx::aio::buffer::pool<int, 100u> pool;

        // Allocate in bursts, release randomly
        for (int cycle = 0; cycle < 3; ++cycle)
        {
            std::vector<kmx::aio::buffer::handle<int>> handles;
            ...
            std::vector<kmx::aio::buffer::handle<int>> kept;
    ...
    TEST_CASE("buffer::pool const access", "[buffer_pool][buffer_handle]")
    {
        kmx::aio::buffer::pool<std::string, 5u> pool;
