...
namespace kmx::aio::quic::test::integration
{
    ...
    TEST_CASE("completion QUIC HTTP3 handshake-stream-response-close smoke", "[quic][http3][integration][smoke][slow]")
    {
        const auto repo_root_opt = find_repo_root();
        REQUIRE(repo_root_opt.has_value());

        const fs::path repo_root = *repo_root_opt;
        const auto server_bin_opt = find_binary_under_debug(repo_root, "sample-quic-http3-server");
        const auto client_bin_opt = find_binary_under_debug(repo_root, "sample-quic-http3-client");

        if (!server_bin_opt.has_value() || !client_bin_opt.has_value())
            SKIP("QUIC HTTP3 smoke skipped: build sample-quic-http3-server and sample-quic-http3-client first");
        ...
        if (!fs::exists(cert_path) || !fs::exists(key_path))
            SKIP("QUIC HTTP3 smoke skipped: missing /tmp/quic_cert.pem or /tmp/quic_key.pem");
