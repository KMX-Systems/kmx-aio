...
namespace kmx::aio::tls::test::integration
{
    ...
    std::optional<std::string> find_binary_under_debug(const std::string& repo_root, const std::string& binary_name)
    {
        if (repo_root.empty())
            return std::nullopt;
    ...
    mtls_cert_set generate_mtls_cert_set(const std::filesystem::path& cert_dir)
    {
        ...
        const auto server_cmd = "openssl req -x509 -newkey rsa:2048 -keyout " + shell_quote(server_key) + " -out " +
                                shell_quote(server_cert) + " -days 1 -nodes -subj '/CN=localhost' >/dev/null 2>&1";
        REQUIRE(std::system(server_cmd.c_str()) == 0);

        // Generate client key
        const auto client_key_cmd = "openssl genrsa -out " + shell_quote(client_key) + " 2048 >/dev/null 2>&1";
        REQUIRE(std::system(client_key_cmd.c_str()) == 0);

        // Generate client CSR
        const auto client_csr_cmd =
            "openssl req -new -key " + shell_quote(client_key) + " -out " + shell_quote(client_csr) + " -subj '/CN=client' >/dev/null 2>&1";
        REQUIRE(std::system(client_csr_cmd.c_str()) == 0);

        // Sign client cert with server key
        const auto sign_cmd = "openssl x509 -req -in " + shell_quote(client_csr) + " -signkey " + shell_quote(server_key) + " -out " +
    ...
    TEST_CASE("mTLS integration: certificate environment validation", "[tls][mtls][integration][smoke]")
    {
        ...
        const auto cert_dir = std::filesystem::path("/tmp/kmx_mtls_cert_validation_test");
        ...
        const auto verify_server_cert = "openssl x509 -in " + shell_quote(certs.server_cert) + " -text -noout >/dev/null 2>&1";
        REQUIRE(std::system(verify_server_cert.c_str()) == 0);

        const auto verify_client_cert = "openssl x509 -in " + shell_quote(certs.client_cert) + " -text -noout >/dev/null 2>&1";
        REQUIRE(std::system(verify_client_cert.c_str()) == 0);

        const auto verify_server_key = "openssl rsa -in " + shell_quote(certs.server_key) + " -check -noout >/dev/null 2>&1";
        REQUIRE(std::system(verify_server_key.c_str()) == 0);

        const auto verify_client_key = "openssl rsa -in " + shell_quote(certs.client_key) + " -check -noout >/dev/null 2>&1";
