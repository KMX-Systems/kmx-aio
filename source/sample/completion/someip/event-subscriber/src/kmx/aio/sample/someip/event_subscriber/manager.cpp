...
{
    manager::manager(kmx::aio::someip::client_config client_config, kmx::aio::someip::subscription_config subscription_config,
    ...
    kmx::aio::task<void> manager::run(kmx::aio::completion::executor& exec, std::shared_ptr<std::atomic_bool> ok) noexcept(false)
    {
        ...
        if (!start_result)
        {
            kmx::logger::log(kmx::logger::level::error, std::source_location::current(), "SOME/IP subscriber client start failed: {}",
        ...
        for (int i = 0; i < 500 && received < expected_events_; ++i)
        {
            ...
            if (!notification)
            {
                ...
                kmx::logger::log(kmx::logger::level::error, std::source_location::current(), "SOME/IP subscriber next failed: {}",
            ...
            kmx::logger::log(kmx::logger::level::info, std::source_location::current(),
                             "SOME/IP subscriber received event #{} event_id=0x{:04x} payload='{}'", received,
                             static_cast<std::uint32_t>(notification->event_id), payload_str);
        }

        auto close_result = co_await subscription_.close();
        if (!close_result)
            kmx::logger::log(kmx::logger::level::warn, std::source_location::current(), "SOME/IP subscriber close failed: {}",
                             close_result.error().message());

        auto release_result = co_await client_.release_service(cfg.service_id, cfg.instance_id);
        if (!release_result)
            kmx::logger::log(kmx::logger::level::warn, std::source_location::current(), "SOME/IP subscriber release_service failed: {}",
                             release_result.error().message());

        auto stop_result = co_await client_.stop();
        if (!stop_result)
            kmx::logger::log(kmx::logger::level::warn, std::source_location::current(), "SOME/IP subscriber stop failed: {}",
        ...
        if (received < expected_events_)
            kmx::logger::log(
                kmx::logger::level::warn, std::source_location::current(),
        ...
        kmx::logger::log(kmx::logger::level::info, std::source_location::current(),
                         "SOME/IP subscriber stats received={} dropped={} expected={}", received, dropped, expected_events_);
