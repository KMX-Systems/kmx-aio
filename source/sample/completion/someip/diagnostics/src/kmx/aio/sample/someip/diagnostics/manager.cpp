...
{
    manager::manager(kmx::aio::someip::client_config client_config, kmx::aio::someip::subscription_config subscription_config) noexcept:
    ...
    kmx::aio::task<void> manager::run(kmx::aio::completion::executor& exec, std::shared_ptr<std::atomic_bool> ok) noexcept(false)
    {
        ...
        if (!client_start)
        {
            kmx::logger::log(kmx::logger::level::error, std::source_location::current(), "SOME/IP diagnostics client start failed: {}",
        ...
        const auto call_result = co_await client_.call_method(client_cfg.service_id, client_cfg.instance_id, 0x3333u, std::move(payload));
        if (call_result)
            kmx::logger::log(kmx::logger::level::info, std::source_location::current(),
                             "SOME/IP diagnostics call_method succeeded (response_size={})", call_result->payload.size());
        else
        ...
        for (int i = 0; i < 30; ++i)
        {
            ...
            kmx::logger::log(kmx::logger::level::info, std::source_location::current(),
                             "SOME/IP diagnostics received event #{} event_id=0x{:04x} payload='{}'", received,
                             static_cast<std::uint32_t>(event->event_id), payload_str);
            if (received >= 3u)
                break;
        }
        ...
        kmx::logger::log(kmx::logger::level::info, std::source_location::current(),
                         ...
                         client_stats.start_attempts, client_stats.calls_sent, client_stats.calls_received, client_stats.dropped_events);
        kmx::logger::log(kmx::logger::level::info, std::source_location::current(),
                         "SOME/IP diagnostics subscription received={} dropped={}", received, subscription_.dropped_events());
