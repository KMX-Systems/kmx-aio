...
{
    manager::manager(kmx::aio::someip::client_config config) noexcept: client_(std::move(config))
    {
    }
    ...
    kmx::aio::task<void> manager::run(kmx::aio::completion::executor& exec, std::shared_ptr<std::atomic_bool> ok) noexcept(false)
    {
        ...
        kmx::logger::log(kmx::logger::level::info, std::source_location::current(),
                         "SOME/IP echo client cfg service=0x{:04x} instance=0x{:04x}", static_cast<std::uint32_t>(cfg.service_id),
        ...
        for (int i = 0; i < 200; ++i)
        {
            ...
            if (!tick)
            {
                kmx::logger::log(kmx::logger::level::error, std::source_location::current(), "SOME/IP client iterate failed: {}",
                                 tick.error().message());
                break;
            }
        }
        ...
        if (!release_result)
            kmx::logger::log(kmx::logger::level::warn, std::source_location::current(), "SOME/IP release_service failed: {}",
        ...
        kmx::logger::log(kmx::logger::level::info, std::source_location::current(),
                         "SOME/IP echo client stats calls_sent={} calls_received={} dropped_events={}", stats.calls_sent,
                         stats.calls_received, stats.dropped_events);
