...
{
    manager::manager(kmx::aio::someip::server_config config) noexcept: server_(std::move(config))
    {
    }
    ...
    kmx::aio::task<void> manager::run(kmx::aio::completion::executor& exec, std::shared_ptr<std::atomic_bool> ok) noexcept(false)
    {
        ...
        kmx::logger::log(kmx::logger::level::info, std::source_location::current(),
                         "SOME/IP echo server cfg service=0x{:04x} instance=0x{:04x}", static_cast<std::uint32_t>(cfg.service_id),
        ...
        for (int i = 0; i < 800 && !replied; ++i)
        {
            ...
            if (!request)
            {
                if (request.error() == kmx::aio::someip::error::timed_out)
                {
                    ...
                    if (!tick)
                    {
                        kmx::logger::log(kmx::logger::level::error, std::source_location::current(), "SOME/IP iterate failed: {}",
                                         tick.error().message());
                        break;
                    }
                    continue;
                ...
                kmx::logger::log(kmx::logger::level::error, std::source_location::current(), "SOME/IP next_request failed: {}",
            ...
            if (!response)
            {
                kmx::logger::log(kmx::logger::level::error, std::source_location::current(), "SOME/IP send_response failed: {}",
        ...
        if (!stop_offer_result)
            kmx::logger::log(kmx::logger::level::warn, std::source_location::current(), "SOME/IP stop_offer_service failed: {}",
                             stop_offer_result.error().message());

        const auto stop_result = co_await server_.stop();
        if (!stop_result)
            kmx::logger::log(kmx::logger::level::warn, std::source_location::current(), "SOME/IP server stop failed: {}",
                             stop_result.error().message());

        const auto& stats = server_.get_stats();
        kmx::logger::log(kmx::logger::level::info, std::source_location::current(),
                         "SOME/IP echo server stats calls_received={} calls_sent={} events_sent={}", stats.calls_received, stats.calls_sent,
                         stats.events_sent);
