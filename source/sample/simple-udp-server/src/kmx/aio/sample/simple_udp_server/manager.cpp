#include "kmx/aio/sample/simple_udp_server/manager.hpp"

#include <arpa/inet.h>
#include <netinet/in.h>
#include <sys/socket.h>

namespace kmx::aio::sample::simple_udp_server
{
    bool manager::run() noexcept(false)
    {
        ...
        logger::log(logger::level::info, std::source_location::current(),
                    "═══════════════════════════════════════════════════════════════");
        logger::log(logger::level::info, std::source_location::current(), "Server: Started. Bound to {}:{}", config_.bind_address,
                    config_.bind_port);
    ...
    {
        // Create UDP endpoint
        logger::log(logger::level::info, std::source_location::current(), "Server: Creating UDP endpoint on {}:{}", config_.bind_address,
                    config_.bind_port);

        auto ep_result = kmx::aio::udp::endpoint::create(*executor_, AF_INET);
        ...
        if (::inet_pton(AF_INET, config_.bind_address.data(), &addr.sin_addr) <= 0)
        {
            logger::log(logger::level::error, std::source_location::current(), "Server: Invalid bind address format");
            co_return;
        }

        if (::bind(ep.raw().get_fd(), reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) < 0)
        {
