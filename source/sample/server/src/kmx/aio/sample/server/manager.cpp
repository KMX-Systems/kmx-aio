...
namespace kmx::aio::sample::server
{
    ...
    kmx::aio::task<void> manager::handle_client(kmx::aio::tcp::stream stream, const std::uint64_t client_id) noexcept(false)
    {
        ...
        stats_.active_connections.fetch_add(1u, std::memory_order_relaxed);
            ...
            while (true)
            {
                ...
                if (!read_result)
                {
                    if (read_result.error().value() != 0)
                    {
                        ...
                        stats_.errors.fetch_add(1u, std::memory_order_relaxed);
                    }
                    break;
                }
                ...
                ++messages_received;
                stats_.bytes_received.fetch_add(bytes_read, std::memory_order_relaxed);
                ...
                ++messages_sent;
                stats_.bytes_sent.fetch_add(*write_result, std::memory_order_relaxed);
    ...
    kmx::aio::task<void> manager::connection_acceptor() noexcept(false)
    {
        ...
        try
        {
            while (true)
            {
                ...
                ++accept_count;
                stats_.total_connections.fetch_add(1u, std::memory_order_relaxed);
                const auto client_id = ++client_counter;

                logger::log(logger::level::info, std::source_location::current(),
                            "Acceptor: Accepted connection #{} (total: {}, active: {})", client_id, stats_.total_connections.load(),
                            stats_.active_connections.load());
    ...
    {
        const auto total_conn = stats_.total_connections.load(std::memory_order_relaxed);
        const auto active_conn = stats_.active_connections.load(std::memory_order_relaxed);
        const auto bytes_recv = stats_.bytes_received.load(std::memory_order_relaxed);
        const auto bytes_sent = stats_.bytes_sent.load(std::memory_order_relaxed);
        const auto errors = stats_.errors.load(std::memory_order_relaxed);
