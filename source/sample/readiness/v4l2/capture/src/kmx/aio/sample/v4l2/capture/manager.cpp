...
namespace kmx::aio::sample::v4l2::capture
{
    ...
    bool manager::run() noexcept(false)
    {
        ...
        kmx::logger::log(kmx::logger::level::info, std::source_location::current(), "Opening V4L2 capture device: {} ({}x{} @ {}/{} fps)",
                         config_.device, requested_size.width, requested_size.height, requested_fps.denominator, requested_fps.numerator);
        ...
        kmx::logger::log(kmx::logger::level::info, std::source_location::current(), "Capture running. Press Ctrl+C to stop.");
        executor_->run();
    ...
    kmx::aio::task<void> manager::capture_loop() noexcept(false)
    {
        ...
        kmx::logger::log(kmx::logger::level::info, std::source_location::current(), "Streaming started: {}x{} FourCC=0x{:08X}, {} buffers",
                         active_size.width, active_size.height, active_format.fourcc, active_cfg.buffer_count);
