/// @file src/kmx/aio/sample/tcp/echo/server/manager.cpp
/// @brief Readiness-model TCP echo server: accept loop, per-client reader and random-data sender, live stats.
...
namespace kmx::aio::sample::tcp::echo::server
{
    ...
    bool manager::run() noexcept(false)
    {
        ...
        logger::log(logger::level::info, std::source_location::current(),
                    "═══════════════════════════════════════════════════════════════");
        logger::log(logger::level::info, std::source_location::current(), "Server: Started. Listening on {}:{}", bind_ip,
                    config_.bind_port);
    ...
    kmx::aio::task<void> manager::handle_client(kmx::aio::readiness::tcp::stream stream, const std::uint64_t client_id,
                                                std::shared_ptr<connection_stats> stats) noexcept(false)
    {
        ...
        auto stream_ptr = std::make_shared<kmx::aio::readiness::tcp::stream>(std::move(stream));
    ...
    kmx::aio::task<void> manager::connection_acceptor() noexcept(false)
    {
        ...
        const auto bind_ip = kmx::aio::ip_to_string(config_.bind_address);
        logger::log(logger::level::info, std::source_location::current(), "Acceptor: Creating listener on {}:{}", bind_ip,
                    config_.bind_port);
