/// @file src/kmx/aio/sample/udp/echo/client/manager.cpp
/// @brief Readiness-model UDP echo client: per-worker send/receive loops over UDP endpoints and a results summary.
...
namespace kmx::aio::sample::udp::echo::client
{
    ...
    task<void> manager::worker(const std::uint32_t worker_id) noexcept(false)
    {
        try
        {
            auto ep_result = kmx::aio::readiness::udp::endpoint::create(*executor_, ip_family(config_.server_address));
            if (!ep_result)
            {
