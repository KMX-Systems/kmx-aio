...
namespace kmx::aio::sample::tls::echo_readiness_server
{
    ...
    kmx::aio::task<void> manager::listener_task() noexcept(false)
    {
        ...
        while (true)
        {
            ...
            auto tcp_stream = kmx::aio::readiness::tcp::stream(*executor_, std::move(*accept_result));
            auto tls_stream = kmx::aio::readiness::tls::stream(std::move(tcp_stream), ssl_ctx_);

            executor_->spawn(handle_client(std::move(tls_stream)));
        }
    }

    kmx::aio::task<void> manager::handle_client(kmx::aio::readiness::tls::stream stream) noexcept(false)
    {
        ...
        try
        {
            ...
            while (true)
            {
                auto read_result = co_await stream.read(span_char_t(buffer));
                if (!read_result)
                {
    ...
    void manager::ui_loop(std::stop_token stop_token) const
    {
        ...
        while (!stop_token.stop_requested())
        {
            ...
            std::cout << "Live TLS Readiness Stats\n";
            std::cout << "────────────────────────────────────────────────────────────────────────\n";
    ...
    {
        std::cout << "\nShutdown Metrics for TLS Readiness\n";
