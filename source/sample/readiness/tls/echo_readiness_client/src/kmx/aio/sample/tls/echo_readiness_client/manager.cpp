...
namespace kmx::aio::sample::tls::echo_readiness_client
{
    ...
    task<std::expected<kmx::aio::readiness::tls::stream, std::error_code>> manager::async_connect() noexcept
    {
        ...
        // If connect is in progress, wait for socket to be writable. A false result is a cancelled wait
        // rather than a ready socket - the executor is shutting the operation down, so the descriptor has
        // to be handed back before leaving.
        if (in_progress)
        ...
        co_return kmx::aio::readiness::tls::stream(kmx::aio::readiness::tcp::stream(*executor_, std::move(fd_owner)), ssl_ctx_);
    }
    ...
    task<void> manager::worker(const std::uint32_t worker_id, std::shared_ptr<connection_stats> stats) noexcept(false)
    {
        ...
        try
        {
            ...
            {
                logger::log(logger::level::error, std::source_location::current(), "Worker [{}]: Handshake failed: {}", worker_id,
                            hs.error().message());
                co_return;
            }
    ...
    kmx::aio::task<void> manager::worker_sender(std::shared_ptr<kmx::aio::readiness::tls::stream> stream, const std::uint32_t worker_id,
                                                std::shared_ptr<connection_stats> stats) noexcept(false)
    {
        try
        {
            ...
            ::shutdown(stream->inner().get_fd(), SHUT_WR);
