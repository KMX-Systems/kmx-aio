#include "kmx/aio/sample/simple_udp_client/manager.hpp"

#include <arpa/inet.h>
#include <netinet/in.h>
#include <sys/socket.h>

namespace kmx::aio::sample::simple_udp_client
{
    ...
    task<void> manager::worker(const std::uint32_t worker_id) noexcept(false)
    {
        ...
        try
        {
            auto ep_result = udp::endpoint::create(*executor_, AF_INET);
            if (!ep_result)
            {
            ...
            if (::inet_pton(AF_INET, config_.server_addr.data(), &server_addr.sin_addr) <= 0)
            {
            ...
            if (auto send_result = co_await ep.send(message_span, reinterpret_cast<const sockaddr*>(&server_addr), sizeof(server_addr));
                !send_result)
            {
            ...
            logger::log(logger::level::debug, std::source_location::current(), "Worker [{}]: Request sent to {}:{}", worker_id,
                        config_.server_addr, config_.server_port);
