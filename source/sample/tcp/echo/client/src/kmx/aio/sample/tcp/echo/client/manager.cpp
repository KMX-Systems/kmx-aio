...
namespace kmx::aio::sample::tcp::echo::client
{
    ...
    task<std::expected<kmx::aio::tcp::stream, std::error_code>> manager::async_connect() noexcept
    {
    ...
    task<void> manager::worker(const std::uint32_t worker_id, std::shared_ptr<connection_stats> stats) noexcept(false)
    {
        ...
        try
        {
            ...
            auto stream_ptr = std::make_shared<kmx::aio::tcp::stream>(std::move(*stream_result));
            if (stats)
            {
    ...
    kmx::aio::task<void> manager::worker_sender(std::shared_ptr<kmx::aio::tcp::stream> stream, const std::uint32_t worker_id,
