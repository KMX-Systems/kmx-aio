...
namespace kmx::aio::knx
{
    ...
    [[nodiscard]] static expected_void_t validate_connect_request(const ipv6_hpai& control, const ipv6_hpai& data, const bool stream) noexcept
    {
    ...
    generic_server::generic_server(datagram_transport& transport, const server_config config, const server_clock_now_function clock_now,
                                   const secure::monotonic_ms_function secure_clock_ms, secure::entropy_source* const entropy) noexcept(false):
        generic_server(config, clock_now, secure_clock_ms, entropy)
    {
        transport_ = &transport;
    }

    generic_server::generic_server(const server_config config, const server_clock_now_function clock_now,
                                   const secure::monotonic_ms_function secure_clock_ms, secure::entropy_source* const entropy) noexcept(false):
        config_(config), clock_now_(clock_now), secure_clock_ms_(secure_clock_ms)
    {
        if (config_.max_channels == 0u)
            config_.max_channels = 1u;
        if (config_.secure != nullptr)
            sessions_ = std::make_unique<secure::server_session_table>(config_.secure,
                                                                       (entropy != nullptr) ? *entropy : secure::system_entropy());
    }
        ...
        const std::optional<individual_address>& requested) const noexcept
    {
        ...
        {
            const auto slot = static_cast<std::int32_t>(requested->value()) - static_cast<std::int32_t>(config_.first_assigned_address.value()) + 1;
    ...
    bool generic_server::peer_matches(const channel& value, const origin& from, const bool data_endpoint) const noexcept
    {
        ...
        if (!value.active || (value.transport != from.transport) || (expected_peer.length == 0u) ||
            (expected_peer.length != peer.length) || (expected_peer.address.ss_family != peer.address.ss_family))
            return false;
    ...
    task_returning_expected_void_t generic_server::send_datagram(const cspan_uint8_t packet, const origin& to) noexcept(false)
    {
        ...
        const auto sent = co_await to.transport->send(
            cspan_byte_t {bytes, packet.size()}, reinterpret_cast<const sockaddr*>(&to.peer.address), to.peer.length);
        if (!sent)
    ...
    server_event_task_t generic_server::refuse_connect(const hpai& data_endpoint, const connect_status status,
                                                       const std::error_code reason, const origin& from) noexcept(false)
    {
        ...
        if (const auto encoded = connection::encode_connect_response_packet(
                response, connect_response_frame {0u, status, response_endpoint, {}});
            !encoded.has_value())
            co_return std::unexpected(encoded.error());
        if (const auto sent = co_await send_datagram(response, from); !sent)
            co_return std::unexpected(sent.error());
    ...
    server_event_task_t generic_server::dispatch(const datagram& value, const origin& from) noexcept(false)
    {
        ...
        // name (P1).
        if (!std::holds_alternative<connect_request_frame>(value.payload) && !std::holds_alternative<ipv6_connect_request_frame>(value.payload) &&
            refuses_in_clear(from))
    ...
    task<generic_server::connection_step_t> generic_server::serve_connection_frame(datagram_transport& connection, const span_uint8_t buffer,
                                                                                  const server_event_handler& on_event) noexcept(false)
    {
        if (const auto active = poll(); !active.has_value())
            co_return std::unexpected(active.error());
        ...
        const std::uint8_t channel_id, const cspan_uint8_t cemi_bytes) noexcept(false)
    {
        ...
        if (!claimed.has_value())
            co_return std::unexpected(make_error_code(error::invalid_configuration));
        std::array<std::uint8_t, frame::max_datagram_size> buffer {};
        if (const auto encoded = frame::encode_tunnelling_request_packet(buffer, channel_id, claimed->sequence, cemi_bytes); !encoded.has_value())
            co_return std::unexpected(encoded.error());
