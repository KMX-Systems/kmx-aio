...
namespace kmx::aio::knx::secure::detail
{
    ...
    struct cipher_context_deleter
    {
        void operator()(EVP_CIPHER_CTX* const context) const noexcept { EVP_CIPHER_CTX_free(context); }
    };
    struct key_deleter
    {
        void operator()(EVP_PKEY* const key) const noexcept { EVP_PKEY_free(key); }
    };
    struct key_context_deleter
    {
        void operator()(EVP_PKEY_CTX* const context) const noexcept { EVP_PKEY_CTX_free(context); }
    };

    using cipher_context_t = std::unique_ptr<EVP_CIPHER_CTX, cipher_context_deleter>;
    using backend_key_t = std::unique_ptr<EVP_PKEY, key_deleter>;
    using key_context_t = std::unique_ptr<EVP_PKEY_CTX, key_context_deleter>;
