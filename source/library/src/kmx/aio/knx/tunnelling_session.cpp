/// @file src/kmx/aio/knx/tunnelling_session.cpp
...
namespace kmx::aio::knx
{
    ...
    expected_void_t tunnelling_session::start_connect_raw(const cspan_uint8_t packet, const std::uint32_t deadline_ms) noexcept
    {
        if (state_ != session_state::idle)
            return std::unexpected(make_error_code(error::invalid_configuration));
    ...
    expected_void_t tunnelling_session::prepare_disconnect_request_packet(const span_uint8_t packet) noexcept
    {
        ...
        const auto encoded = connection::encode_disconnect_request_packet(
            packet, disconnect_request_frame {static_cast<std::uint8_t>(channel_id_), control_endpoint_});
        if (!encoded.has_value())
            return std::unexpected(encoded.error());
    ...
    expected_void_t tunnelling_session::prepare_tunnelling_ack_packet(const span_uint8_t packet, const tunnelling_request_frame& request,
    ...
    expected_void_t tunnelling_session::prepare_tunnelling_ack_packet(const span_uint8_t packet, const cspan_uint8_t request_packet,
                                                                      const std::uint8_t status) const noexcept
    {
    ...
    expected_void_t tunnelling_session::prepare_response_datagram(const span_uint8_t packet, const cspan_uint8_t request_packet,
                                                                  const std::uint8_t status) noexcept
    {
    ...
    expected_void_t tunnelling_session::dispatch_session_datagram(const datagram& value, const std::uint32_t now_ms) noexcept
    {
    ...
    expected_void_t tunnelling_session::dispatch_session_datagram(const cspan_uint8_t packet, const std::uint32_t now_ms) noexcept
    {
