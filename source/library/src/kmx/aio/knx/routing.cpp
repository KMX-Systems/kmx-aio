...
namespace kmx::aio::knx::routing
{
    namespace detail
    {
        ...
        {
            secure_routing(secure_configuration value, const secure::monotonic_ms_function clock,
    ...
    [[nodiscard]] static constexpr bool is_routing_service(const std::uint16_t service) noexcept
    {
    ...
    client::client(datagram_transport& transport, const multicast_configuration configuration, secure_configuration secure_settings,
                   const secure::monotonic_ms_function clock_ms, secure::entropy_source* const entropy,
                   const clock_now_function clock_now) noexcept(false):
        transport_(transport),
        configuration_(configuration),
        clock_now_(clock_now),
        secure_(std::make_unique<detail::secure_routing>(std::move(secure_settings), clock_ms,
    ...
    {
        auto& state = *secure_;
        const auto wrapper = secure::decode_secure_wrapper_packet(packet);
        ...
        if (!inner.has_value() || !is_routing_service(inner->service_type))
        {
