/// @file src/kmx/aio/avb/gptp/primary_clock.cpp
...
namespace kmx::aio::avb::gptp
{
    ...
    tai_timestamp_t primary_clock::clock_tai_now() noexcept
    {
        ::timespec ts {};
        ::clock_gettime(CLOCK_TAI, &ts);
        return static_cast<tai_timestamp_t>(ts.tv_sec) * 1'000'000'000ULL + static_cast<tai_timestamp_t>(ts.tv_nsec);
    }

    // Build a minimal gPTP header

    header primary_clock::make_header(const msg_type t, const std::uint16_t len, const std::uint16_t seq_id) const noexcept
    {
        header h {};
        h.set_type(t);
        h.version_ptp = 0x02u;
    ...
    void primary_clock::dispatch(const std::byte* const data, const std::size_t len, const tai_timestamp_t rx_hw_ts) noexcept
    {
        if (len < sizeof(header))
            return;

        const auto* const hdr = reinterpret_cast<const header*>(data);
        switch (hdr->type())
        {
            case msg_type::sync:
    ...
    void primary_clock::on_sync(const std::byte* const data, const std::size_t len, const tai_timestamp_t rx_hw_ts) noexcept
    {
    ...
    void primary_clock::on_follow_up(const std::byte* const data, const std::size_t len) noexcept
    {
        ...
        const tai_timestamp_t t1 = f->body.precise_origin_timestamp.to_ns();
    ...
    void primary_clock::on_pdelay_resp(const std::byte* const data, const std::size_t len, const tai_timestamp_t rx_hw_ts) noexcept
    {
    ...
    void primary_clock::on_announce(const std::byte* const data, const std::size_t len) noexcept
    {
        ...
        const auto* const h = reinterpret_cast<const header*>(data);
        if (!gm_id_.has_value())
