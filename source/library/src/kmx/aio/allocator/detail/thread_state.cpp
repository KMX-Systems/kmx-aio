...
namespace kmx::aio::allocator::detail
{
    ...
    std::uint64_t total_allocations(const counter_kind kind) noexcept
    {
        std::uint64_t total {};
        const std::lock_guard lock(g_statistics_mutex);
        for (const auto* state = g_thread_states; state != nullptr; state = state->next)
            total += (kind == counter_kind::slab) ? state->slab_allocations.load(std::memory_order_relaxed) :
    ...
    {
        const std::lock_guard lock(g_statistics_mutex);
        for (auto* state = g_thread_states; state != nullptr; state = state->next)
        {
