/// @file src/kmx/aio/quic/basic_endpoint.cpp
/// @brief Packet loop, engine setup and lsquic callbacks of the QUIC endpoint.
...
namespace kmx::aio::quic
{
    ...
    task<std::expected<stream, std::error_code>> basic_endpoint::open_stream() noexcept(false)
    {
        ...
        pending_opens_ += 1u;
        if (conn_ != nullptr)
            static_cast<void>(::lsquic_conn_make_stream(conn_));
    ...
    void basic_endpoint::drain_socket(std::vector<char>& packet, const ::sockaddr_storage& local) noexcept
    {
        for (;;)
        {
            ...
            ++packets_in_;
            static_cast<void>(::lsquic_engine_packet_in(engine_, reinterpret_cast<const unsigned char*>(packet.data()),
    ...
    void basic_endpoint::release_waiters() noexcept
    {
        ...
        static_cast<void>(::shutdown(socket_.get(), SHUT_RDWR));
    ...
    task<void> basic_endpoint::run() noexcept(false)
    {
        ...
        ::sockaddr_storage local {};
        ::socklen_t local_len = sizeof(local);
        static_cast<void>(::getsockname(socket_.get(), reinterpret_cast<::sockaddr*>(&local), &local_len));
    ...
    {
        static_cast<void>(co_await io_timeout(duration_ns));
        signal_wakeup();
        co_return;
    }
    ...
    {
        static_cast<void>(co_await io_poll(socket_.get(), POLLIN));
        poll_armed_ = false;
        signal_wakeup();
        co_return;
    }
    ...
    void basic_endpoint::cb_hsk_done(::lsquic_conn_t* const conn, const enum lsquic_hsk_status status) noexcept
    {
        ...
        for (std::size_t i = 0u; i != self->pending_opens_; ++i)
            static_cast<void>(::lsquic_conn_make_stream(conn));
    }
    ...
    void basic_endpoint::cb_write(::lsquic_stream_t* const handle, ::lsquic_stream_ctx_t* const ctx) noexcept
    {
        ...
        static_cast<void>(::lsquic_stream_flush(handle));
