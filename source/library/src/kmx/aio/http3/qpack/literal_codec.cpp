/// @file src/kmx/aio/http3/qpack/literal_codec.cpp
/// @brief Literal-only QPACK header codec using the static table, with no dynamic table.
...
namespace kmx::aio::http3::qpack
{
    namespace detail
    {
        [[nodiscard]] std::error_code decode_error() noexcept
        {
    ...
    [[nodiscard]] static std::size_t estimated_block_size(const header_list& headers, const std::vector<lookup_result>& lookups) noexcept
    {
    ...
    [[nodiscard]] static std::optional<std::pair<std::string, std::string>> read_field(const cspan_uint8_t payload, std::size_t& offset,
                                                                                       const field_representation representation) noexcept
    {
    ...
    {
        if (payload.size() < 2u)
            return std::unexpected(detail::decode_error());
