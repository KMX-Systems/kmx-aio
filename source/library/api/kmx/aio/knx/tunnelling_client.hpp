/// @file api/kmx/aio/knx/tunnelling_client.hpp
...
namespace kmx::aio::knx
{
    ...
    /// @brief How a tunnelling client times its exchanges, and the clock it reads.
    struct tunnelling_client_options
    {
        /// @brief The tunnelling timing and retry policy.
        tunnelling_config config {};
        /// @brief The monotonic millisecond clock; the steady clock when null.
        clock_now_function clock_now {};
    };

    /// @brief What a tunnelling client opens a KNX IP Secure session with, and what the session runs on.
    struct secure_tunnelling_options
    {
        /// @brief The user id, keys and serial number the session is opened with.
        secure::tunnelling_credentials credentials {};
        /// @brief The tunnelling timing and retry policy, and the monotonic millisecond clock of the tunnel.
        tunnelling_client_options tunnel {};
        /// @brief The monotonic clock the session's keep-alive and timeout run on; the steady clock when null.
        secure::monotonic_ms_function clock_ms {};
        /// @brief Where session key pairs come from; the system source when null. Must outlive the client.
    ...
    public:
        ...
        /// @param options The tunnelling timing and retry policy, and the monotonic millisecond clock.
        ...
        tunnelling_client(datagram_transport& transport, const sockaddr* peer, ::socklen_t peer_length,
                          tunnelling_client_options options = {}) noexcept;

        /// @brief Creates a client from a peer the caller already holds as `sockaddr_storage`.
        /// @copydetails tunnelling_client
        tunnelling_client(datagram_transport& transport, const sockaddr_storage& peer, const ::socklen_t peer_length,
                          const tunnelling_client_options options = {}) noexcept:
            tunnelling_client(transport, reinterpret_cast<const sockaddr*>(&peer), peer_length, options)
        {
        }
        ...
        /// @param options The credentials the session is opened with, the tunnelling policy and clock, and the clock and
        ///        entropy the session runs on.
        ...
        [[nodiscard]] task_returning_expected_void_t connect(const connect_request_frame& request) noexcept(false);

        /// @brief Opens a tunnelling connection over IPv6.
        /// @param request The connection to ask for.
        /// @return A task yielding nothing, or the reason no connection was established.
        [[nodiscard]] task_returning_expected_void_t connect(const ipv6_connect_request_frame& request) noexcept(false);
        ...
        [[nodiscard]] task_returning_expected_void_t send(cspan_uint8_t cemi_bytes) noexcept(false);
    ...
    private:
        ...
        template <typename Step>
        [[nodiscard]] std::invoke_result_t<Step, const tunnelling_session&> with_session(Step&& step) const noexcept
        {
            const std::lock_guard lock {session_mutex_};
        ...
        [[nodiscard]] task_returning_expected_void_t send_packet(cspan_uint8_t packet, endpoint_kind kind) noexcept(false);
        ...
        [[nodiscard]] bool is_retransmission(const expected_void_t& accepted, const tunnelling_request_frame& request) const noexcept;
