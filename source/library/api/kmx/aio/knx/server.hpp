...
namespace kmx::aio::knx
{
    ...
    public:
        ...
        /// @param secure_clock_ms The clock secure sessions time out on; the steady clock when null.
        /// @param entropy Where session key pairs come from; @ref kmx::aio::knx::secure::system_entropy when null. Must
        ///        outlive the server.
        /// @throws std::bad_alloc when the secure sessions cannot be allocated.
        generic_server(datagram_transport& transport, server_config config = {}, server_clock_now_function clock_now = nullptr,
                       secure::monotonic_ms_function secure_clock_ms = nullptr, secure::entropy_source* entropy = nullptr) noexcept(false);
        ...
        explicit generic_server(server_config config, server_clock_now_function clock_now = nullptr,
                                secure::monotonic_ms_function secure_clock_ms = nullptr,
    ...
    private:
        ...
        std::invoke_result_t<Step> with_table(Step&& step) const noexcept(std::is_nothrow_invocable_v<Step>)
        {
        ...
        [[nodiscard]] server_event_task_t refuse_ipv6_connect(const ipv6_hpai& data_endpoint, connect_status status,
                                                              std::error_code reason, const origin& from) noexcept(false);
        ...
        [[nodiscard]] server_event_task_t answer_search(
            const discovery::search_request_frame& request, const origin& from) noexcept(false);
        [[nodiscard]] server_event_task_t answer_extended_search(
            const discovery::extended_search_request_frame& request, const origin& from) noexcept(false);
        ...
        [[nodiscard]] server_event_task_t answer_description(
            const discovery::description_request_frame& request, const origin& from) noexcept(false);
        ...
        [[nodiscard]] task<connection_step_t> serve_secure_frame(secure_connection& state, const server_event_handler& on_event) noexcept(false);
        ...
        [[nodiscard]] task<connection_step_t> accept_wrapper(secure_connection& state, const secure::secure_wrapper_frame& wrapper,
                                                            const transport_peer& peer, const server_event_handler& on_event) noexcept(false);
        /// @brief Serves a frame a session carried, handing a tunnelled one to @p on_event.
        [[nodiscard]] task<connection_step_t> serve_session_frame(secure::session_link& link, cspan_uint8_t packet, const transport_peer& peer,
                                                                 const server_event_handler& on_event) noexcept(false);
