...
namespace kmx::aio::avb::avtp
{
    ...
    [[nodiscard]] inline std::uint32_t to_avtp_timestamp_32(const avb_timestamp_t tai_ns) noexcept
    {
        return static_cast<std::uint32_t>(tai_ns & 0xFFFF'FFFFu);
    }

    [[nodiscard]] inline avb_timestamp_t expand_avtp_timestamp_32(const std::uint32_t ts32, const avb_timestamp_t reference_ns) noexcept
    {
    ...
    [[nodiscard]] inline std::expected<std::vector<std::byte>, std::error_code> build_am824_frame(
        const stream_id_t& stream_id, const std::uint8_t sequence_num, const avb_timestamp_t presentation_time_ns,
    ...
    [[nodiscard]] inline std::expected<am824_frame_view, std::error_code> parse_am824_frame(std::span<const std::byte> frame) noexcept
    {
