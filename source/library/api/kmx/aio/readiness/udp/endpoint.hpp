/// @file api/kmx/aio/readiness/udp/endpoint.hpp
...
namespace kmx::aio::readiness::udp
{
    ...
    public:
        ...
        using expected_t = std::expected<endpoint, std::error_code>;
        ...
        [[nodiscard]] static expected_t create(executor& exec, const int domain = AF_INET) noexcept;
        ...
        [[nodiscard]] task_returning_expected_size_t recv_until(span_byte_t buffer, sockaddr_storage& peer_addr,
                                                                ::socklen_t& out_peer_addr_len, std::uint32_t deadline_ms) noexcept(false);

        /// @brief Receive a datagram and decode the sender's IP address and port in addition to its raw socket address.
        /// @param buffer Destination byte span for payload data.
        /// @param out_peer_address Receives the sender's socket address and its length.
        /// @param out_peer Receives the sender's IP address and UDP port, decoded from @p out_peer_address.
        /// @return Awaitable task producing bytes received, or an error: EINVAL when the sender's address is
