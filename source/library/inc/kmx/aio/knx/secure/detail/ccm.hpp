/// @file kmx/aio/knx/secure/detail/ccm.hpp
...
namespace kmx::aio::knx::secure::detail
{
    ...
    /// @param key The key to MAC under.
    ...
    [[nodiscard]] mac_result_t cbc_mac(const crypto_backend& backend, const secret_key& key, const block_t& block_0,
                                       cspan_uint8_t associated_data, cspan_uint8_t payload) noexcept;
    ...
    [[nodiscard]] expected_void_t ctr(const crypto_backend& backend, const secret_key& key, const block_t& counter_0, span_uint8_t mac,
                                      span_uint8_t payload) noexcept;

    /// @brief MACs then encrypts: the payload is encrypted in place and the encrypted MAC is returned.
    /// @param backend The primitives to use.
    /// @param key The key.
    /// @param block_0 B0.
    /// @param counter_0 The first counter block.
    /// @param associated_data A.
    /// @param payload P on entry, its ciphertext on return.
    /// @return The encrypted MAC.
    [[nodiscard]] mac_result_t seal(const crypto_backend& backend, const secret_key& key, const block_t& block_0, const block_t& counter_0,
                                    cspan_uint8_t associated_data, span_uint8_t payload) noexcept;

    /// @brief Decrypts and verifies: the payload is decrypted in place, then the MAC is checked.
    /// @param backend The primitives to use.
    /// @param key The key.
    /// @param block_0 B0, built from the received fields.
    /// @param counter_0 The first counter block, built from the received fields.
    /// @param associated_data A, as received.
    /// @param payload The ciphertext on entry; the plaintext on success, all zero on failure.
    ...
    ///         was altered.
    /// @retval kmx::aio::knx::error::crypto_failure The backend failed.
    [[nodiscard]] expected_void_t open(const crypto_backend& backend, const secret_key& key, const block_t& block_0,
                                       const block_t& counter_0, cspan_uint8_t associated_data, span_uint8_t payload,
                                       const block_t& mac) noexcept;
