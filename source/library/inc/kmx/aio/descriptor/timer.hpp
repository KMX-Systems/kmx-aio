...
namespace kmx::aio::descriptor
{
    ...
    public:
        ...
        [[nodiscard]] static std::expected<timer, std::error_code> create(int clockid = CLOCK_MONOTONIC,
                                                                          int flags = TFD_NONBLOCK | TFD_CLOEXEC) noexcept;
        ...
        [[nodiscard]] std::expected<void, std::error_code> set_time(int flags, const struct itimerspec& new_value,
                                                                    struct itimerspec* old_value = nullptr) noexcept;
