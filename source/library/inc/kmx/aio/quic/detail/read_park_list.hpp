/// @file inc/kmx/aio/quic/detail/read_park_list.hpp
...
namespace kmx::aio::quic::detail
{
    ...
    public:
        ...
        /// @note A stream opens an episode exactly when it is newly inserted into a list it is then alone in;
        ///       parking a stream that is already parked never does.
        bool park(::lsquic_stream* const stream) noexcept(false) { return streams_.insert(stream).second && (streams_.size() == 1u); }
